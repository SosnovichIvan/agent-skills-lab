#!/usr/bin/env python3
"""External compile/static gate for one IAM quality benchmark chunk."""
...
def evaluate_markers(
    ...
    if ast_facts is not None:
        apply_ast_markers(marker_results, ast_facts)
    return marker_results


def collect_ast_facts(project: Path, timeout: int) -> tuple[dict[str, Any] | None, str | None]:
    helper = Path(__file__).with_name("go_ast_facts.go")
    completed = subprocess.run(
        ["go", "run", str(helper), "--project", str(project)],
        cwd=project,
        text=True,
        capture_output=True,
        timeout=timeout,
    ...
    if completed.returncode != 0:
        return None, (completed.stderr or completed.stdout)[-2000:]
    try:
        value = json.loads(completed.stdout)
    except json.JSONDecodeError as error:
        return None, str(error)
    return (value, None) if isinstance(value, dict) else (None, "AST facts must be an object")


def apply_ast_markers(results: dict[str, dict[str, bool]], facts: dict[str, Any]) -> None:
    types = {str(item.get("name", "")).lower() for item in (facts.get("types") or []) if isinstance(item, dict)}
    functions = {str(value).lower() for value in (facts.get("functions") or [])}
    methods = {str(value).lower() for value in (facts.get("methods") or [])}
    strings = {str(value).lower() for value in (facts.get("strings") or [])}
    selectors = {str(value).lower() for value in (facts.get("selectors") or [])}

    def set_marker(task_id: str, name: str, value: bool) -> None:
        if task_id in results and name in results[task_id]:
            results[task_id][name] = value

    set_marker("1.3", "user model", "user" in types)
    set_marker("1.3", "public profile", bool(types & {"publicuser", "publicprofile", "userprofile"}))
    set_marker("4.1", "session model", "session" in types)
    set_marker("6.1", "organization model", "organization" in types)
    set_marker("6.1", "membership model", "membership" in types)
    set_marker("6.3", "invite model", bool(types & {"invite", "invitetoken", "organizationinvite"}))
    set_marker("7.1", "custom role", bool(types & {"customrole", "organizationrole", "role"}))

    route_roles = any("/roles" in value for value in strings)
    set_marker("7.2", "role route discriminator", route_roles)
    set_marker(
        "7.2",
        "role assignment handler",
        "assignrole" in functions | methods or (route_roles and "http.methodput" in selectors),
    )
    set_marker(
        "7.2",
        "role revocation handler",
        bool({"revokerole", "removerole"} & (functions | methods))
        or (route_roles and "http.methoddelete" in selectors),
    )
...
def verify(project: Path, task_id: str, final: bool, timeout: int) -> dict[str, Any]:
    ...
    ast_facts, ast_error = collect_ast_facts(project, timeout)
    marker_results = evaluate_markers(combined, marker_ids, ast_facts)
