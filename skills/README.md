...
| `execution-state/SKILL.md` | [`SKILL.md`](execution-state/SKILL.md) | Выполнение через состояние | Универсальный explicit-only координатор для любого agent CLI: выбирает passthrough/lite/reset, ведёт компактный state и передаёт semantic chunks через capability-based adapters. |
