...
class StateCtlTests(unittest.TestCase):
    ...
    def architecture_review_json(
        self,
        *,
        verdict: str = "passed",
        blockers: list[dict] | None = None,
        summary: str = "Architecture contracts remain valid",
        planned_gaps: list[str] | None = None,
        recommendations: list[str] | None = None,
    ) -> str:
        return json.dumps(
            {
                "protocol": statectl.ARCHITECTURE_REVIEW_PROTOCOL,
                "verdict": verdict,
                "summary": summary,
                "blockers": blockers or [],
                "planned_gaps": planned_gaps or [],
                "recommendations": recommendations or [],
                "checks": [
                    {
                        "id": "architecture-contracts",
                        "status": "passed" if verdict == "passed" else "failed",
                        "summary": "Reviewed dependency direction and public contracts",
                    }
                ],
            },
            ensure_ascii=False,
    ...
    def test_version_matches_release_manifest(self) -> None:
        code, result = self.run_cli("version")
        self.assertEqual(0, code, result)
        release = json.loads(
            (SCRIPTS.parent / "release.json").read_text(encoding="utf-8")
        )
        self.assertEqual(release, {key: value for key, value in result.items() if key not in {"ok", "command"}})
    ...
    def test_runtime_plan_rejects_noncanonical_worker_v3_fields(self) -> None:
        ...
        self.assertEqual(0, code, result)
        canonical = json.loads(packet_path.read_text(encoding="utf-8"))
        prompt_path = self.root / "worker.md"
        prompt_path.write_text("Use the attached worker packet.", encoding="utf-8")

        invalid_packets = (
            {**canonical, "revision": canonical["based_on_revision"]},
            {key: value for key, value in canonical.items() if key != "state_id"},
            {**canonical, "language_policy": {"operational": "ru", "source_content": "preserve"}},
        )
    ...
    def test_periodic_architecture_review_blocks_next_packet(self) -> None:
        ...
        code, result = self.run_cli(
            ...
            "--expected-revision", "2", "--review-json", self.architecture_review_json(),
        )
        self.assertEqual(0, code, result)
        code, result = self.run_cli(
            "packet", *self.state_arguments(), "--output", str(packet_path)
    ...
    def test_cross_area_change_requires_integration_chunk(self) -> None:
        ...
        code, result = self.run_cli(
            ...
            "--review-json", self.architecture_review_json(),
        )
        self.assertEqual(0, code, result)
        code, result = self.run_cli(
    ...
    def test_architecture_review_compacts_multibyte_summary_and_evidence(self) -> None:
        ...
        code, result = self.run_cli(
            ...
            "--expected-revision", "2", "--review-json", self.architecture_review_json(
                summary=summary,
                planned_gaps=[evidence],
                recommendations=["Keep the dependency graph acyclic"],
            ),
        )
        self.assertEqual(0, code, result)
        ...
        compact = json.loads(last_review)
        self.assertEqual("passed", compact["verdict"])
        self.assertIn("blockers", compact)
        self.assertIn("planned_gaps", compact)
        self.assertIn("recommendations", compact)
        self.assertIn("checks", compact)

    def test_architecture_review_recovery_requires_re_review_before_normal_task(self) -> None:
        path = self.init_standalone(task_json=[
            {"id": "one", "title": "First", "done_when": ["first works"]},
            {"id": "two", "title": "Second", "done_when": ["second works"]},
            {"id": "three", "title": "Third", "done_when": ["third works"]},
        ])
        state = self.load_state(path)
        ...
        for revision in (0, 1):
            code, result = self.run_cli(
                "complete", *self.state_arguments(), "--expected-revision", str(revision),
                "--summary", "done", "--evidence", "check passed",
            )
            self.assertEqual(0, code, result)
        blocker = {
            "contract": "Existing login route must remain wired",
            "evidence": "POST /login returns 404",
            "affected_files": ["internal/http/routes.go"],
            "regression_check": "login-route-regression",
        }
        code, result = self.run_cli(
            "architecture-review", *self.state_arguments(), "--expected-revision", "2",
            "--review-json", self.architecture_review_json(
                verdict="blocked",
                blockers=[blocker],
                planned_gaps=["Metrics are scheduled for task 8.3"],
            ),
        )
        self.assertEqual(0, code, result)
        self.assertEqual("blocked", result["verdict"])
        recovery = result["recovery_task"]
        self.assertEqual([blocker["contract"]], recovery["contracts"])
        self.assertEqual([blocker["regression_check"]], recovery["regression_checks"])

        packet_path = self.root / "recovery.json"
        code, packet_result = self.run_cli(
            "packet", *self.state_arguments(), "--output", str(packet_path)
        )
        self.assertEqual(0, code, packet_result)
        packet = json.loads(packet_path.read_text(encoding="utf-8"))
        self.assertEqual(recovery["id"], packet["task"]["id"])
        code, completion = self.run_cli(
            "complete", *self.state_arguments(),
            "--expected-revision", str(packet_result["revision"]),
            "--run-id", packet["run_id"],
            "--summary", "login route restored",
            "--check-json", json.dumps({
                "id": blocker["regression_check"],
                "status": "passed",
                "summary": "POST /login is wired",
            }),
        )
        self.assertEqual(0, code, completion)
        self.assertTrue(completion["review_required"])
        code, blocked_packet = self.run_cli(
            "packet", *self.state_arguments(), "--output", str(self.root / "normal.json")
        )
        self.assertEqual(statectl.ERROR_INVALID, code, blocked_packet)
        code, rereview = self.run_cli(
            "architecture-review", *self.state_arguments(),
            "--expected-revision", str(completion["revision"]),
            "--review-json", self.architecture_review_json(),
        )
        self.assertEqual(0, code, rereview)
        self.assertEqual("reset", self.load_state(path)["quality"]["next_handoff"])
        code, packet_result = self.run_cli(
            "packet", *self.state_arguments(), "--output", str(self.root / "normal.json")
        )
        self.assertEqual(0, code, packet_result)
        self.assertEqual("three", json.loads((self.root / "normal.json").read_text())["task"]["id"])
